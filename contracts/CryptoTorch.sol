 ...
 * it will not prevent player dividend withdraws
 */
 ...
 * see https://github.com/OwnTheDayIO/otdtc
 */
 ...
 * see https://github.com/OwnTheDayIO/cttc
 */
contract CryptoTorchToken {
    ...
    function exitFor(address _for) public;
...
contract CryptoTorch is Pausable, ReentrancyGuard {
     ...
     *  and cash-out any dividends from both Torch & Tokens
     */
    function exit() public {
        address _for = msg.sender;

        // Cashout Torch Dividends
        cashoutFor_(_for);

        // Sell Tokens and Cashout all Token Dividends
        CryptoTorchToken_.exitFor(_for);
    }

    /**
     * Sell some tokens and cashout the earned Token Dividends to Ether
     *  - this includes any Token Referral Bonuses
     */
    ...
    /**
     * Cash-out the earned Torch Dividends to Ether
     *  - Does not touch Token Dividends or Token Referral Bonuses
     */
    function cashout() public returns (uint256) {
        return cashoutFor_(msg.sender);
    }
    ...
    function handlePayouts_(uint256 _forDev, uint256 _forPayout, uint256 _forDayOwner, address _takenBy, address _previousLast, address _dayOwner) internal {
        ...
        // Transfer Funds
        //  - Transfer directly since these accounts are, or may not be, existing
     ...
     *  - Does not touch Token Dividends or Token Referral Bonuses
     */
    function cashoutFor_(address _for) internal returns (uint256) {
