 ...
 *
 * Token System for the Crypto-Torch
 *
...
contract CryptoTorchToken is ERC20, Ownable {
    ...
    uint256 constant internal dividendFee_ = 2;
    ...
    /**
     * Sell all tokens (use sell(amount) to specify an amount to sell)
     */
    function exit() public onlyTokenHolders {
        exit_(msg.sender);
    }
     ...
     * Sell all tokens (use sell(amount) to specify an amount to sell)
     */
    function exitFor(address _for) public onlyTokenController {
        exit_(_for);
    }
    ...
    function calculateTokensReceived(uint256 _etherToSpend) public view returns(uint256) {
        ...
        //uint256 _referralFee = (_hasReferral) ? _dividends.div(10) : 0;
        // contract test 2
        //uint256 _referralFee = (_hasReferral) ? _etherToSpend.div(dividendFee_).div(10) : 0;
        //contract test 3
        uint256 _dividends = _etherToSpend.div(dividendFee_);
     ...
     * Sell all tokens (use sell(amount) to specify an amount to sell)
     */
    function exit_(address _recipient) internal {
        require(tokenBalanceLedger_[_recipient] > 0);

        // get token count for caller & sell them all
        sell_(_recipient, tokenBalanceLedger_[_recipient]);
    }
     ...
     */
    function withdrawFor_(address _recipient) internal {
    ...
    function etherToTokens_(uint256 _ether) internal view returns(uint256) {
        ...
        /* uint256 _tokenPriceInitial = tokenPriceInitial_ * 1e18;
        uint256 _tokensReceived = ((SafeMath.sub((sqrt((_tokenPriceInitial**2) + (2*(tokenPriceIncremental_ * 1e18)*(_ether * 1e18)) + (((tokenPriceIncremental_)**2)*(tokenSupply_**2)) + (2*(tokenPriceIncremental_)*_tokenPriceInitial*tokenSupply_))), _tokenPriceInitial)) / (tokenPriceIncremental_)) - (tokenSupply_);
        return _tokensReceived; */
    }
